...
public class HumiditySensor extends BaseInstanceEnabler implements Destroyable {
    ...
    public HumiditySensor() {
        // this.scheduler = Executors.newSingleThreadScheduledExecutor(new NamedThreadFactory("HumiditySensor"));
    }

    public HumiditySensor(String value) {
        humidity = Double.parseDouble(value);
    }

    @Override
    public synchronized ReadResponse read(LwM2mServer server, int resourceId) {
        LOG.info("Read on HumiditySensor resource /{}/{}/{}", getModel().id, getId(), resourceId);
        switch (resourceId) {
        case SENSOR_VALUE:
            return ReadResponse.success(resourceId, getHumidity());
        default:
    ...
    public boolean setValue(String value) {
        if (Double.parseDouble(value) != humidity) {
