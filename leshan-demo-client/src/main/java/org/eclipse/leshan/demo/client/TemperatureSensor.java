...
public class TemperatureSensor extends BaseInstanceEnabler implements Destroyable {
    ...
    public TemperatureSensor() {
        // this.scheduler = Executors.newSingleThreadScheduledExecutor(new NamedThreadFactory("TemperatureSensor"));
    }
    ...
    public synchronized ReadResponse read(LwM2mServer server, int resourceId) {
        LOG.info("Read on TemperatureSensor resource /{}/{}/{}", getModel().id, getId(), resourceId);
        switch (resourceId) {
        case SENSOR_VALUE:
    ...
    public boolean setValue(String value) {
        if (Double.parseDouble(value) != temperature) {
            temperature = Double.parseDouble(value);
            return true;
        }
        return false;
